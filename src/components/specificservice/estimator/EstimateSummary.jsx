...
export default function EstimateSummary({
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="custom-scrollbar mt-4 max-h-[200px] flex-1 space-y-2 overflow-y-auto pr-1">
        {selectedItems.length === 0 ? (
          <p className="py-4 text-center text-xs text-zinc-500">Select features to see estimate</p>
        ) : (
          selectedItems.map((item) => (
            <div
              key={item.id}
              className="truncate rounded-lg border border-zinc-800 bg-[#101012] px-3 py-2 text-xs text-zinc-300"
            >
              {item.label}
            </div>
          ))
      ...
      <div className="mt-4 border-t border-zinc-800 pt-4">
        ...
        <a
          href={mailtoHref}
          className="mt-2 block rounded-xl border border-zinc-800 bg-[#1b1b1e] py-2.5 text-center text-xs font-semibold text-zinc-300 transition-colors hover:border-zinc-600"
        >
