...
import { Compass, Users, Rocket, Flag, ArrowRight, Sparkles } from "lucide-react";
...
const principles = [
  {
    ...
    subtitle: "Proven Discovery Process",
    description:
      "We dig until the problem is concrete. Scope, constraints, and the real bottleneck get nailed down before a single line of code. No assumptions, no guesswork — just clarity.",
    image: "/images/philosophy/discovery.jpeg",
    accent: "#f472b6",
  },
  {
    icon: Users,
    title: "Senior Craft, Zero Handoffs",
    subtitle: "Value-Add Showcase",
    description:
      "The people who scope your project are the ones who build it. No BDRs, no juniors, no surprise bench swaps. Senior expertise from first call to final deploy.",
    image: "/images/philosophy/senior-team.jpeg",
    accent: "#60a5fa",
  },
  {
    icon: Rocket,
    title: "Ship to Learn",
    subtitle: "Built For Growth & Scale",
    description:
      "Working software in front of real users beats speculation every time. We move in data-driven sprints and let feedback steer. From MVP to enterprise — flexible systems that grow.",
    image: "/images/philosophy/ship-learn.jpeg",
    accent: "#4ade80",
  },
  {
    icon: Flag,
    title: "Own the Outcome",
    subtitle: "Quick to Market, Built to Last",
    description:
      "We co-own the result. Quality, timeline, and business impact are our problem — not just the ticket list. Your success is our metric for done.",
    image: "/images/philosophy/own-outcome.jpeg",
    accent: "#c084fc",
  },
];
...
export default function PhilosophyInteractive() {
  ...
  const handleItemClick = (index) => {
    ...
    setTimeout(() => {
      ...
      setProgress(0);
      setIsTransitioning(false);
      setImageError(false); // Reset error on manual change
    }, 300);
  };
      ...
      <div className="container-custom relative">
        <div className="mb-12 lg:mb-16">
          ...
          <h2 className="mt-4 max-w-xl text-3xl font-bold tracking-[-0.02em] sm:text-4xl">
            How Kivro Helps{" "}
            <span className="text-[#60a5fa]">Businesses Stand Out</span>
          </h2>
        </div>
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1fr_1.4fr]">
          ...
          <div className="rounded-[20px] border border-white/10 bg-white/[0.03] p-6 sm:p-8">
            ...
            <div className="relative mb-6 flex overflow-hidden rounded-xl border border-white/10 bg-black/30">
              {imageError ? (
                // Fallback: Show colored box with icon if image fails
                <div
                  className="flex h-[300px] w-full items-center justify-center sm:h-[320px]"
                  style={{ backgroundColor: `${activePrinciple.accent}20` }}
                >
                  <ActiveIcon
                    className="h-20 w-20"
                    style={{ color: activePrinciple.accent }}
                    strokeWidth={1.5}
                  />
                </div>
              ) : (
              ...
              <span
                ...
                style={{
                  backgroundColor: activePrinciple.accent,
                  boxShadow: `0 10px 30px ${activePrinciple.accent}60`,
                }}
              >
            ...
            <div className="mt-3 flex items-center justify-between text-xs font-medium text-white/50">
              <span>
                Step {activeIndex + 1} of {principles.length}
              </span>
              <span>
                Auto-advances in{" "}
                {Math.max(
        ...
        <div className="mt-12 flex flex-wrap items-center justify-between gap-6 rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:p-8">
          <div className="flex flex-wrap items-center gap-4 sm:gap-6">
            ...
            <p className="max-w-md text-sm leading-relaxed text-white/80">
              Our philosophy-driven approach bridges these gaps — delivering
              software that actually ships and scales.
            </p>
          </div>
